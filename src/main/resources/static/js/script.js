...
if(document.getElementById("form-lloyd")){ 
  document.getElementById("form-lloyd").addEventListener("submit", async function(event) {
    event.preventDefault();
    ...
    const response = await fetch("/clasificar-lloyd", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
    });
...
if(document.getElementById("btn-ejemplos-lloyd")){

  document.getElementById("btn-ejemplos-lloyd").addEventListener("click", async function () {
    const ejemplos = [
      { x1: 5.1, x2: 3.5, x3: 1.4, x4: 0.2 },
      { x1: 6.0, x2: 2.7, x3: 4.5, x4: 1.5 },
      { x1: 5.8, x2: 2.7, x3: 3.9, x4: 1.2 }
    ...
    const resultados = await Promise.all(ejemplos.map(async muestra => {
      const res = await fetch("/clasificar-lloyd", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(muestra)
      });
    ...
    document.getElementById("ejemplos-container-lloyd").innerHTML = html;
  });
